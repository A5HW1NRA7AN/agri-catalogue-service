...
public class SoilController {
    ...
    public ResponseEntity<CustomResponse> draft(@RequestBody JsonNode soilDetails) {
        lifecyclePolicy.requireEnabled(CATALOGUE_NAME);
        CustomResponse response = soilService.draftSoil(soilDetails);
        return new ResponseEntity<>(response, response.getResponseCode());
    }
    ...
    @PostMapping("/v1/add")
    public ResponseEntity<CustomResponse> add(@RequestBody JsonNode soilDetails) {
        CustomResponse response = soilService.createSoil(soilDetails);
        return new ResponseEntity<>(response, response.getResponseCode());
    }
    ...
    public ResponseEntity<CustomResponse> addById(@PathVariable String id, @RequestBody JsonNode soilDetails) {
        lifecyclePolicy.requireEnabled(CATALOGUE_NAME);
        CustomResponse response = soilService.addSoil(id, soilDetails);
        return new ResponseEntity<>(response, response.getResponseCode());
    }
    ...
    public ResponseEntity<CustomResponse> approve(@RequestBody LifecycleRequest request) {
        lifecyclePolicy.requireEnabled(CATALOGUE_NAME);
        CustomResponse response = soilService.approveSoil(request);
        return new ResponseEntity<>(response, response.getResponseCode());
    }
    ...
    public ResponseEntity<CustomResponse> review(@RequestBody LifecycleRequest request) {
        lifecyclePolicy.requireEnabled(CATALOGUE_NAME);
        CustomResponse response = soilService.reviewSoil(request);
        return new ResponseEntity<>(response, response.getResponseCode());
    }
    ...
    public ResponseEntity<CustomResponse> toggle(@PathVariable String id) {
        CustomResponse response = soilService.toggleStatus(id);
        return new ResponseEntity<>(response, response.getResponseCode());
    }
    ...
    public ResponseEntity<?> delete(@PathVariable String id) {
        CustomResponse response = soilService.delete(id);
        return new ResponseEntity<>(response, HttpStatus.OK);
    }
