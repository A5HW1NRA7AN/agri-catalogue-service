...
public class SeedController {
    ...
    public ResponseEntity<CustomResponse> draft(@RequestBody JsonNode seedDetails) {
        CustomResponse response = seedService.draftSeed(seedDetails);
        return new ResponseEntity<>(response, response.getResponseCode());
    }

    // Lifecycle: create a new record submitted for approval (PENDING, full validation)
    @PostMapping("/v1/add")
    public ResponseEntity<CustomResponse> add(@RequestBody JsonNode seedDetails) {
        CustomResponse response = seedService.createSeed(seedDetails);
        return new ResponseEntity<>(response, response.getResponseCode());
    }
    ...
    public ResponseEntity<CustomResponse> addById(@PathVariable String id, @RequestBody JsonNode seedDetails) {
        CustomResponse response = seedService.addSeed(id, seedDetails);
        return new ResponseEntity<>(response, response.getResponseCode());
    }
    ...
    public ResponseEntity<CustomResponse> approve(@RequestBody LifecycleRequest request) {
        CustomResponse response = seedService.approveSeed(request);
        return new ResponseEntity<>(response, response.getResponseCode());
    }
    ...
    public ResponseEntity<CustomResponse> review(@RequestBody LifecycleRequest request) {
        CustomResponse response = seedService.reviewSeed(request);
        return new ResponseEntity<>(response, response.getResponseCode());
    }
    ...
    public ResponseEntity<CustomResponse> toggle(@PathVariable String id) {
        CustomResponse response = seedService.toggleStatus(id);
        return new ResponseEntity<>(response, response.getResponseCode());
    }
